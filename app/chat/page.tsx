  ...
  return (
    <div style={{
      ...
      <style>{`
        ...
        * { margin:0; padding:0; box-sizing:border-box; }
        ...
        @media (max-width: 600px) {
          ...
          .model-label { display: none; } /* hide sub-label on small screens */
          .chat-ta { font-size: 16px; padding: 12px 14px; }
          .send-btn { width: 38px; height: 38px; margin: 6px; }
          .msg-max { max-width: 90% !important; }
          .avatar { width: 26px !important; height: 26px !important; font-size: 11px !important; }
          .hint-text { display: none; }
          .suggestions-wrap { padding: 0 4px; }
        }
      `}</style>
              ...
              >
                <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
                  ...
                  <div style={{ textAlign: "left" }}>
                    <div style={{ fontSize: 12, fontWeight: 600, color: model === m.value ? m.color : "#555", lineHeight: 1.2 }}>
                      {m.provider}
                    </div>
                    <div className="model-label" style={{ fontSize: 10, color: model === m.value ? `${m.color}99` : "#333", lineHeight: 1.2 }}>
                      {m.label}
                    </div>
                  </div>
                  <div style={{
                    fontSize: 8,
                    fontWeight: 700,
                    color: m.tag === "FREE" ? "#4ade80" : "#f97316",
                    background: m.tag === "FREE" ? "rgba(74,222,128,0.1)" : "rgba(249,115,22,0.1)",
                    padding: "2px 4px",
                    borderRadius: 3,
                    marginLeft: 2,
                  }}>{m.tag}</div>
                ...
                {model === m.value && (
                  <div style={{ width: "100%", height: 2, background: m.color, borderRadius: 2, marginTop: 3 }} />
                )}
              </button>
            ))}
          </div>
        ...
        <div style={{ maxWidth: 800, margin: "0 auto", display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          {messages.length === 0 && (
            <div style={{ textAlign: "center", padding: "40px 0" }}>
              ...
              <div style={{ fontSize: 18, fontWeight: 700, color: "#222", marginBottom: 6, letterSpacing: "-0.02em" }}>
                What can I help you with?
              </div>
              <div style={{ fontSize: 13, color: "#333", marginBottom: 28 }}>
                Using{" "}
                <span style={{ color: currentModel.color, fontWeight: 600 }}>
                  {currentModel.icon} {currentModel.provider} {currentModel.label}
                </span>
              </div>
              <div className="suggestions-wrap" style={{ display: "flex", gap: 8, justifyContent: "center", flexWrap: "wrap", maxWidth: 560, margin: "0 auto" }}>
                {["Write a Python script", "Explain black holes", "Create a business plan", "Debug my code"].map((s) => (
              ...
              >
                {msg.role === "assistant" && (
                  <div className="avatar" style={{
                    ...
                    background: `${msgModel.color}15`,
                    border: `1px solid ${msgModel.color}30`,
      ...
      <div style={{
        ...
        backdropFilter: "blur(16px)",
        flexShrink: 0,
        paddingBottom: "max(12px, env(safe-area-inset-bottom))", /* iPhone notch safe area */
      }}>
        <div style={{ maxWidth: 800, margin: "0 auto" }}>
          <div style={{
            background: "#0d0d0d",
            ...
            <button
              ...
              style={{
                background: input.trim() && !loading ? currentModel.color : "#1a1a1a",
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginTop: 6, padding: "0 4px" }}>
            ...
            <span style={{ fontSize: 11, color: currentModel.color, opacity: 0.6 }}>
              {currentModel.icon} {currentModel.provider} {currentModel.label}
            </span>
          </div>
        </div>
