...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // ==================== NVIDIA (DEEPSEEK) ====================
    if (model === "nvidia" || model === "deepseek") {
      const NVIDIA_KEY = process.env.NVIDIA_API_KEY;

      if (!NVIDIA_KEY) {
        return NextResponse.json({
          reply: "⚠️ NVIDIA API key missing!",
        });
      }
  ...
  max_tokens: 2048
      ...
      if (!response.ok) {
        return NextResponse.json({
          reply:
            "❌ NVIDIA error: " + (data?.error?.message || "Unknown error"),
        });
      }
    ...
    if (!response.ok) {
      if (data?.error?.code === 429) {
        return NextResponse.json({
          reply:
            "⚠️ Gemini busy hai! Groq pe switch karo — fast aur free ⚡",
        });
      }
